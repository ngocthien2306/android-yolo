...
static Yolo_Detect* g_yolo = 0;
static ncnn::Mutex lock;
static ImageClassifier* classifier = 0;
...
void MyNdkCamera::on_image_render(cv::Mat& rgb) const
{
    // nanodet
    {
        ...
        if (classifier) {
            std::pair<int, std::pair<std::string, float>> result = classifier->inference(rgb);
            std::string className = result.second.first;
            float score = result.second.second;
            std::string predictionText = className + " conf: " + std::to_string(score);
            cv::putText(rgb, predictionText, cv::Point(20, 20),
...
JNIEXPORT jboolean JNICALL Java_com_tencent_yolov8ncnn_Yolov8Ncnn_loadModel(JNIEnv* env, jobject thiz, jobject assetManager, jint modelid, jint cpugpu)
{
    ...
    // reload
    {
        ...
        {
            if (!g_yolo)
                g_yolo = new Yolo_Detect;

//            if (!classifier) {
//                classifier = new ImageClassifier;
//                classifier->loadLabels(mgr,"imagenet_classes.txt");
//            }

            // classifier->loadModel(mgr, modeltype, 224);
